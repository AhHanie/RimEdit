...
pub(crate) fn scan_indexable_patch_xml_files(
    ...
    for folder in &resolution.selected_folders {
        ...
        for result in WalkDir::new(&patches_dir).follow_links(false) {
            ...
            if should_shadow {
                // Identity key: the folder's content-pack scope (distinct per Workshop item for
                // a SteamWorkshop collection, see `ResolvedLoadFolder::scope`) plus the path
                // relative to the load folder (e.g. `Patches/Foo.xml`). Two different content
                // packs never shadow each other.
